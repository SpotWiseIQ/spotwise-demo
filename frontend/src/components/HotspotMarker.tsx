...
import { Cloud, Sun, Footprints, Wind, CloudRain, ChevronDown, ChevronUp, BarChart2, MapPin, Users } from "lucide-react";
...
export const HotspotMarker: React.FC<HotspotMarkerProps> = ({
  ...
  const weatherInfo = getWeatherIcon(hotspot.weather?.toLowerCase() || 'unknown');
    ...
    >
      ...
      >
        <div className="flex items-center min-h-[40px]">
          <div className={`hotspot-marker ${hotspot.trafficLevel?.toLowerCase()} !h-6 !w-6 !text-xs !flex !items-center !justify-center`}>
            {hotspot.label}
          </div>
          <div className="ml-3 flex-grow h-[20px] flex items-center overflow-hidden">
            <p className="text-sm font-medium truncate" title={`Zone ${hotspot.label}`}>{`Zone ${hotspot.label}`}</p>
          </div>
          <div className="flex flex-col min-w-[110px]">
            <div className="grid grid-cols-2 gap-x-5">
              <div className="flex flex-col items-center">
                <div className={`w-6 h-6 ${getTrafficColor(hotspot.trafficLevel?.toLowerCase() || 'unknown')} rounded-md flex items-center justify-center text-white`}>
      ...
      {/* Expanded Detail Section (Conditionally Rendered) - MOVED UP */}
      {selected && (
          ...
          <div>
            ...
            <div className="space-y-1.5 text-sm">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                ...
                <div className="flex items-center">
                  <MapPin size={14} className="mr-1.5 text-gray-500 flex-shrink-0" />
                  <span className="text-gray-500 text-xs mr-1">Area Type:</span>
                  <span>{hotspot.areaType || 'N/A'}</span>
                </div>
                <div className="flex items-center col-span-2">
                  <Users size={14} className="mr-1.5 text-gray-500 flex-shrink-0" />
                  <span className="text-gray-500 text-xs mr-1">Demographics:</span>
      ...
      {/* Foot Traffic Collapsible (independent toggle) - NOW LAST ELEMENT */}
      <Collapsible
        open={isTrafficOpen}
